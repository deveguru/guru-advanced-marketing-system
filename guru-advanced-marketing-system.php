...
final class Guru_Advanced_Marketing_System_Final {
    ...
    private function render_tab_links($user_id) { global $wpdb; $products = wc_get_products(['limit' => -1]); $links = $wpdb->get_results($wpdb->prepare("SELECT * FROM {$this->db_links_table} WHERE marketer_id = %d ORDER BY created_at DESC", $user_id)); ob_start(); ?> <h2 class="gams-h2"><?php esc_html_e('مدیریت لینک‌ها', self::TEXT_DOMAIN); ?></h2> <div class="gams-card"><h3 class="gams-card-title"><i class="bi bi-plus-circle-fill"></i> <?php esc_html_e('ساخت لینک جدید', self::TEXT_DOMAIN); ?></h3><form method="POST" action=""><input type="hidden" name="gams_action" value="generate_link"><?php wp_nonce_field('gams_dashboard_action', 'gams_action_nonce'); ?><div class="gams-form-grid"><div><label class="gams-label"><?php esc_html_e('محصول (اختیاری)', self::TEXT_DOMAIN); ?></label><select name="product_id" class="gams-input"><option value="0"><?php esc_html_e('لینک کلی سایت', self::TEXT_DOMAIN); ?></option><?php foreach ($products as $product) { echo '<option value="'.esc_attr($product->get_id()).'">'.esc_html($product->get_name()).'</option>'; } ?></select></div><div><label class="gams-label"><?php esc_html_e('نامک سفارشی (اختیاری)', self::TEXT_DOMAIN); ?></label><input type="text" name="custom_slug" placeholder="<?php esc_attr_e('مثال: off-yalda', self::TEXT_DOMAIN); ?>" class="gams-input"></div><div><label class="gams-label"><?php esc_html_e('انقضا پس از (روز)', self::TEXT_DOMAIN); ?></label><input type="number" name="expire_in_days" min="0" value="0" placeholder="<?php esc_attr_e('0 برای نامحدود', self::TEXT_DOMAIN); ?>" class="gams-input"></div><div><label class="gams-label"><?php esc_html_e('حداکثر استفاده', self::TEXT_DOMAIN); ?></label><input type="number" name="max_uses" min="0" value="0" placeholder="<?php esc_attr_e('0 برای نامحدود', self::TEXT_DOMAIN); ?>" class="gams-input"></div></div><button type="submit" class="gams-button"><?php esc_html_e('ساخت لینک', self::TEXT_DOMAIN); ?></button></form></div> <div class="gams-card"><h3 class="gams-card-title"><i class="bi bi-list-task"></i> <?php esc_html_e('لینک‌های ساخته شده', self::TEXT_DOMAIN); ?></h3><div class="gams-table-wrapper"><table class="gams-table"><thead><tr><th><?php esc_html_e('لینک', self::TEXT_DOMAIN); ?></th><th><?php esc_html_e('محصول هدف', self::TEXT_DOMAIN); ?></th><th><?php esc_html_e('کلیک', self::TEXT_DOMAIN); ?></th><th><?php esc_html_e('انقضا', self::TEXT_DOMAIN); ?></th><th><?php esc_html_e('عملیات', self::TEXT_DOMAIN); ?></th></tr></thead><tbody> <?php if(empty($links)): ?><tr><td colspan="5"><?php esc_html_e('هنوز هیچ لینکی نساخته‌اید.', self::TEXT_DOMAIN); ?></td></tr><?php else: foreach ($links as $link): $link_url = add_query_arg([ 'utm_source' => self::UTM_SOURCE_VAL, self::UTM_CAMPAIGN_VAR => $link->marketer_id, self::UTM_CONTENT_VAR => $link->custom_slug ], home_url('/')); ?> <tr id="link-row-<?php echo esc_attr($link->link_id); ?>"><td><input type="text" readonly value="<?php echo esc_url($link_url); ?>" id="link-<?php echo esc_attr($link->link_id); ?>" class="gams-input-readonly"></td><td><?php echo $link->product_id ? esc_html(get_the_title($link->product_id)) : esc_html__('عمومی', self::TEXT_DOMAIN); ?></td><td><?php echo esc_html($link->use_count); ?></td><td><?php echo $link->expire_at ? esc_html(date_i18n('Y/m/d', strtotime($link->expire_at))) : esc_html__('ندارد', self::TEXT_DOMAIN); ?></td><td> <button class="gams-button-small" onclick="gamsCopy('link-<?php echo esc_attr($link->link_id); ?>')"><i class="bi bi-clipboard"></i></button><button class="gams-button-small" onclick="showGamsQrCode('<?php echo esc_js($link_url); ?>')"><i class="bi bi-qr-code"></i></button><button class="gams-button-small" onclick="toggleEditForm('<?php echo esc_attr($link->link_id); ?>')"><i class="bi bi-pencil-square"></i></button> <form method="POST" action="" style="display:inline;" onsubmit="return confirm('<?php echo esc_js(__('آیا از حذف این لینک مطمئن هستید؟', self::TEXT_DOMAIN)); ?>');"><input type="hidden" name="gams_action" value="delete_link"><input type="hidden" name="link_id" value="<?php echo esc_attr($link->link_id); ?>"><?php wp_nonce_field('gams_dashboard_action', 'gams_action_nonce'); ?><button type="submit" class="gams-button-small gams-button-danger"><i class="bi bi-trash"></i></button></form> </td></tr> <tr id="edit-form-row-<?php echo esc_attr($link->link_id); ?>" style="display:none;"><td colspan="5"><form method="POST" action="" class="gams-edit-form"><input type="hidden" name="gams_action" value="update_link"><input type="hidden" name="link_id" value="<?php echo esc_attr($link->link_id); ?>"><?php wp_nonce_field('gams_dashboard_action', 'gams_action_nonce'); ?><div class="gams-form-grid"> <div><label><?php esc_html_e('محصول', self::TEXT_DOMAIN); ?></label><select name="product_id" class="gams-input"><?php echo '<option value="0" '.selected(0, $link->product_id, false).'>' . esc_html__('لینک کلی سایت', self::TEXT_DOMAIN) . '</option>'; foreach ($products as $product) { echo '<option value="'.esc_attr($product->get_id()).'" '.selected($product->get_id(), $link->product_id, false).'>'.esc_html($product->get_name()).'</option>'; } ?></select></div> <div><label><?php esc_html_e('نامک سفارشی', self::TEXT_DOMAIN); ?></label><input type="text" name="custom_slug" value="<?php echo esc_attr($link->custom_slug); ?>" class="gams-input"></div> <div><label><?php esc_html_e('انقضا (روز)', self::TEXT_DOMAIN); ?></label><input type="number" name="expire_in_days" value="<?php echo $link->expire_at ? floor((strtotime($link->expire_at) - time())/DAY_IN_SECONDS) : 0; ?>" min="0" class="gams-input"></div> <div><label><?php esc_html_e('حداکثر استفاده', self::TEXT_DOMAIN); ?></label><input type="number" name="max_uses" value="<?php echo esc_attr($link->max_uses); ?>" min="0" class="gams-input"></div> </div><button type="submit" class="gams-button"><?php esc_html_e('ذخیره تغییرات', self::TEXT_DOMAIN); ?></button><button type="button" class="gams-button gams-button-secondary" onclick="toggleEditForm('<?php echo esc_attr($link->link_id); ?>')"><?php esc_html_e('انصراف', self::TEXT_DOMAIN); ?></button></form></td></tr> <?php endforeach; endif; ?></tbody></table></div></div> <?php return ob_get_clean(); }
